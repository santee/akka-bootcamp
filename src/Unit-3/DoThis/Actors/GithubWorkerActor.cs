    ...
    public class GithubWorkerActor : ReceiveActor
    {
        ...
        private void InitialReceives()
        {
            ...
            Receive<RetryableQuery>(query => query.Query is QueryStarrer, query =>
            {
                ...
                this._gitHubClient.Activity.Starring.GetAllForUser(starrer).ContinueWith<object>(
                    tr =>
                        {
            ...
            Receive<RetryableQuery>(query => query.Query is QueryStarrers, query =>
            {
                ...
                var sender = this.Sender;
                var getStars = _gitHubClient.Activity.Starring.GetAllStargazers(starrers.Owner, starrers.Repo).ContinueWith<object>(
                    tr =>
                        {
                            if (tr.IsFaulted || tr.IsCanceled) return query.NextTry();
